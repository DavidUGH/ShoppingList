<script lang="ts">
	...
	function editItem(product: Product){
		...
		//TODO: We can see from the logs that the product that's passed
		//to this function is the same as the one passed to the ItemList
		//component. Something's wrong.
		if(idx !== -1){
	...
	<p class="text-3xl">Lista</p>
		<div class="flex flex-col gap-3">
			{#each products as product}
				<ListItem product={product} {editItem}/>
			{/each}
		</div>
		<button
		...
		+</button>
	{#if expand}
	<ItemForm itemFunction={addNewItem}/>
	{/if}
